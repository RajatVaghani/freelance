var formsWsUrl = window.location.protocol + '//formsws';
...
$(document).ready(function () {
	...
	var getFuelType = true; // true if it has default selection
	$('select#hvacSystemType').on('change', function() {
		if ($(this).val() == 'Central Heating' || $(this).val() == 'Boiler') {
			$('.typeOfFuel').show('fast');
			getFuelType = true;
		}
		else {
	...
	function processForm() {
		var formData = $('#steps :input[name!=hvacSystemType]').serializeArray();
    	...
    	var hvacSystemType = $('#hvacSystemType').val()
    	if (getFuelType) {
    		var hvacFuelType = $('#hvacFuelType').val() == null ? ' Natural Gas Fired' : ' ' + $('#hvacFuelType').val();
        	hvacSystemType += hvacFuelType;
    	}

    	formData.push({name: 'hvacSystemType', value: hvacSystemType});
